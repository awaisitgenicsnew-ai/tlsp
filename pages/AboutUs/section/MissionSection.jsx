"use client";

export default function MissionSection() {
  return (
    <section
      className="w-full bg-[#d9d9d9] text-[var(--ink)] min-h-screen flex items-center justify-center px-6 md:px-12 lg:px-20 py-10 lg:py-24"
    >
      <div className="max-w-6xl mx-auto w-full">
        <div className="text-center mb-12">
          <div className="flex items-center justify-center mb-6">
            <span className="h-px bg-[var(--tan)] w-16 mr-6 hidden sm:block"></span>
            <p className="font-sans text-sm tracking-[0.25em] font-medium text-[var(--tan)] whitespace-nowrap uppercase">
              Our Mission
            </p>
            <span className="hidden sm:block h-px bg-[var(--tan)] w-16 ml-6"></span>
          </div>
          <h2 className="font-display text-3xl sm:text-4xl md:text-5xl leading-tight max-w-4xl mx-auto mb-8 text-[#2a2620]">
            Creating Spaces That Inspire
          </h2>
          <p className="font-sans text-sm leading-relaxed text-[var(--ink)]/80 max-w-2xl mx-auto paragraph">
            Our mission is to develop exceptional properties that exceed expectations 
            and set new standards in luxury living across the UAE.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {/* Mission Card 1 */}
          <div className="bg-[#1a1a1a] p-8 border border-transparent hover:border-[var(--tan)] transition-colors">
            <div className="w-12 h-12 bg-[var(--tan)]/20 flex items-center justify-center mb-6">
              <svg className="w-6 h-6 text-[var(--tan)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
              </svg>
            </div>
            <h3 className="font-display text-xl mb-3 text-white">Quality First</h3>
            <p className="font-sans text-sm leading-relaxed text-white/70 paragraph">
              We never compromise on quality. Every material, every finish, 
              every detail is carefully selected to ensure excellence.
            </p>
          </div>

          {/* Mission Card 2 */}
          <div className="bg-[#1a1a1a] p-8 border border-transparent hover:border-[var(--tan)] transition-colors">
            <div className="w-12 h-12 bg-[var(--tan)]/20 flex items-center justify-center mb-6">
              <svg className="w-6 h-6 text-[var(--tan)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
            </div>
            <h3 className="font-display text-xl mb-3 text-white">Innovation</h3>
            <p className="font-sans text-sm leading-relaxed text-white/70 paragraph">
              We embrace cutting-edge technology and innovative design to create 
              living spaces that are ahead of their time.
            </p>
          </div>

          {/* Mission Card 3 */}
          <div className="bg-[#1a1a1a] p-8 border border-transparent hover:border-[var(--tan)] transition-colors">
            <div className="w-12 h-12 bg-[var(--tan)]/20 flex items-center justify-center mb-6">
              <svg className="w-6 h-6 text-[var(--tan)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
              </svg>
            </div>
            <h3 className="font-display text-xl mb-3 text-white">Customer Focus</h3>
            <p className="font-sans text-sm leading-relaxed text-white/70 paragraph">
              Our customers are at the heart of everything we do. We listen, 
              understand, and deliver beyond expectations.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
}
